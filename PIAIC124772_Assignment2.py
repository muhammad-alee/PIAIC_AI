...
def function3():
    ...
    x = a[np.logical_and(a%5 == 0, a%7 == 0)  ]
    return x
    """
    Expected Output:
...
def function7():
    ...
    return z= np.zeros((4),dtype=np.int64)
...
def function8():
    ...
    one =  np.ones((2,5),dtype=np.int32)
    x = np.multiply(one,6)
    return x
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.concatenate((arr,arr,arr)) 
...
def function17():
    ...
    return  np.where(np.logical_and(arr%3 == 0, arr%5 == 0), "YES", "NO")
